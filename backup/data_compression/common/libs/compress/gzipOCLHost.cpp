...
uint64_t gzipOCLHost::decompressEngineSeq(
    ...
    std::cout<<"623 input_size: "<<input_size<<std::endl;
    std::cout<<"624 inBufferSize: "<<inBufferSize<<std::endl;
    std::cout<<"625 isLast: "<<isLast<<std::endl;
    std::cout<<"626 lim_4gb: "<<lim_4gb<<std::endl;
    std::cout<<"627 h_dcompressStatus.size(): "<<h_dcompressStatus.size()<<std::endl;
    std::cout<<"628 h_dcompressStatus.data()[0]: "<<h_dcompressStatus.data()[0]<<std::endl;
    ...
    std::cout<<"671 decmpSizeIdx: "<<decmpSizeIdx<<std::endl;
    std::cout<<"672 h_dcompressStatus.data()[0]: "<<h_dcompressStatus.data()[0]<<std::endl;

    // Copy input data
    std::memcpy(dbuf_in.data(), in, inBufferSize); // must be equal to input_size
    std::cout<<"676 hexdump dbuf_in.data()";
    hexdump(dbuf_in.data(), inBufferSize);
...
// start parallel reader kernel enqueue thread
    ...
    std::cout<<"721 decmpSizeIdx: "<<decmpSizeIdx<<std::endl;
    std::cout<<"722 max_outbuf_size: "<<max_outbuf_size<<std::endl;
    if (decmpSizeIdx >= max_outbuf_size) {
        ...
        std::cout<<"750 hexdump out";
        hexdump(out, decmpSizeIdx);
    }
...
size_t gzipOCLHost::compressEngineSeq(
...
#else
    std::cout<<"1955 GZIP_STREAM not set"<<std::endl;
    std::string compress_kname = compress_kernel_names[2];
#endif
    std::cout<<"1958 compress_kname = "<<compress_kname<<std::endl;
    if (!this->is_freeRunKernel()) {
        std::cout<<"1960 Kernel created"<<std::endl;
        OCL_CHECK(err, m_compressFullKernel = new cl::Kernel(*m_program, compress_kname.c_str(), &err));
    }
    ...
    if (m_zlibFlow) {
        std::cout<<"1979 m_zlibFlow = "<<m_zlibFlow<<std::endl;
        h_buf_checksum_data.data()[0] = 1;
        checksum_type = false;
    } else {
        std::cout<<"1983 m_zlibFlow = "<<m_zlibFlow<<std::endl;
    ...
    if (this->is_freeRunKernel()) {
        std::cout<<"2042 free running kernel migrate kernel arguments"<<std::endl;
        OCL_CHECK(err, err = m_def_q->enqueueMigrateMemObjects({*(buffer_input), *(buffer_output), *(buffer_cSize)},
                                                               0 /* 0 means from host*/));
    } else {
        std::cout<<"2046 not free running kernel migrate kernel arguments"<<std::endl;
...
#endif
        if (this->is_freeRunKernel()) {
            std::cout<<"2059 free running kernel run task"<<std::endl;
            m_def_q->enqueueTask(*datamover_kernel);
        } else {
            std::cout<<"2062 not free running kernel run task"<<std::endl;
    ...
    std::cout<<"2076 migrate compressed size from device"<<std::endl;
    ...
    if (this->is_freeRunKernel()) {
        std::cout<<"2084 not free running kernel migrate result from device"<<std::endl;
        // Transfer the data from device to host
        OCL_CHECK(err, err = m_def_q->enqueueMigrateMemObjects({*(buffer_output)}, CL_MIGRATE_MEM_OBJECT_HOST));
    } else {
        std::cout<<"2088 not free running kernel migrate result from device"<<std::endl;
        // Transfer the data from device to host
    ...
    outIdx += compSizeCntr;
    if (!is_freeRunKernel()) {
        std::cout<<"2134 not free running kernel add last block"<<std::endl;
