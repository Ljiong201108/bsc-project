...
#ifdef SNAPPY_STREAM
    std::cout<<"SNAPPY_STREAM"<<std::endl;
    enbytes = compressEngineStreamSeq(in, out, m_InputSize);
#else
    std::cout<<"not SNAPPY_STREAM"<<std::endl;
...
std::cout<<"DISABLE_FREE_RUNNING_KERNEL defined"<<std::endl;
#undef FREE_RUNNING_KERNEL
#endif
    cl_mem_ext_ptr_t p2pInExt;
    char* p2pPtr = NULL;
    uint32_t inputSize4KMultiple = 0;
    std::cout<<"750 m_enableP2P = "<<m_enableP2P<<std::endl;
    if (m_enableP2P) {
...
std::cout<<"FREE_RUNNING_KERNEL not defined"<<std::endl;
#ifndef DISABLE_FREE_RUNNING_KERNEL
std::cout<<"DISABLE_FREE_RUNNING_KERNEL not defined"<<std::endl;
