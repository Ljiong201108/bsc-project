...
uint64_t gzipBase::xilCompress(uint8_t* in, uint8_t* out, uint64_t input_size) {
    ...
    std::cout<<"121 final_bytes = "<<final_bytes<<std::endl;
    std::cout<<"hexdump out"<<std::endl;
    hexdump(out, final_bytes);

    // GZip Footer
    final_bytes = (this->is_freeRunKernel()) ? final_bytes : writeFooter(out, outIdx + enbytes, checksum);
    std::cout<<"127 final_bytes = "<<final_bytes<<std::endl;
    std::cout<<"hexdump out"<<std::endl;
    hexdump(out, final_bytes);

    std::cout<<"131 End uint64_t gzipBase::xilCompress(uint8_t* in, uint8_t* out, uint64_t input_size)"<<std::endl;
    return final_bytes;
}
