...
ErrCode Aggregator::aggr_sol0(Table& tab_in, Table& tab_out, AggrConfig& aggr_cfg, std::vector<size_t> params) {
    ...
    int agg_i = 0;
    for (int i = 0; i < 8; i++) {
        mext_table_in_col[i] = {(unsigned int)(agg_i++), table_in_col[i], agg_kernel};
    }

    mext_meta_aggr_in = {(unsigned int)(agg_i++), meta_aggr_in.meta(), agg_kernel};
    mext_meta_aggr_out = {(unsigned int)(agg_i++), meta_aggr_out.meta(), agg_kernel};

    for (int i = 0; i < 16; ++i) {
        mext_table_out[i] = {(unsigned int)(agg_i++), table_out_col[i], agg_kernel};
    }
    mext_cfg = {(unsigned int)(agg_i++), table_cfg, agg_kernel};
    mext_cfg_out = {(unsigned int)(agg_i++), table_cfg_out, agg_kernel};
    for (int i = 0; i < 8; i++) {
        memExt[i] = {(unsigned int)(agg_i++), nullptr, agg_kernel};
    }
...
ErrCode Aggregator::aggr_sol1(Table& tab_in, Table& tab_out, AggrConfig& aggr_cfg, std::vector<size_t> params) {
    ...
    for (int k = 0; k < 2; k++) {
        agg_i = 0;
        for (int c = 0; c < 8; c++) {
            mext_tab_aggr_in_col[c][k] = {(unsigned int)(agg_i++), tab_in_col[c][k], aggrkernel[k]};
        }
        mext_meta_aggr_in[k] = {(unsigned int)(agg_i++), meta_aggr_in[k].meta(), aggrkernel[k]};
        mext_meta_aggr_out[k] = {(unsigned int)(agg_i++), meta_aggr_out[k].meta(), aggrkernel[k]};
    }

    for (int k = 0; k < 2; k++) {
        agg_i = 10;
        for (int i = 0; i < 16; ++i) {
            mext_tab_aggr_out[i][k] = {(unsigned int)(agg_i++), tab_aggr_out_col[i][k], aggrkernel[k]};
        }
    }

    mext_cfg_aggr = {(unsigned int)(agg_i++), cfg_aggr, aggrkernel[0]};
    mext_cfg_aggr_out = {(unsigned int)(agg_i++), cfg_aggr_out, aggrkernel[0]};

    for (int c = 0; c < 8; c++) {
        mext_aggr_tmp[c] = {(unsigned int)(agg_i++), nullptr, aggrkernel[0]};
    }
...
ErrCode Aggregator::aggr_sol2(Table& tab_in, Table& tab_out, AggrConfig& aggr_cfg, std::vector<size_t> params) {
    ...
    err = xf::database::gqe::load_binary(&prg, ctx, dev_id, (xclbinPath()+"/gqePart.xclbin").c_str());
    if (err != CL_SUCCESS) {
        fprintf(stderr, "ERROR: fail to program PL\n");
        exit(1);
    }
    ...
    int part_i = 3;
    for (int i = 0; i < 8; ++i) {
        mext_tab_part_in_col[i][0] = {(unsigned int)(part_i), tab_part_in_col[i][0], partkernel[0]};
        mext_tab_part_in_col[i][1] = {(unsigned int)(part_i++), tab_part_in_col[i][1], partkernel[1]};
    }

    mext_meta_part_in[0] = {(unsigned int)(part_i), meta_part_in[0].meta(), partkernel[0]};
    mext_meta_part_in[1] = {(unsigned int)(part_i++), meta_part_in[1].meta(), partkernel[1]};
    mext_meta_part_out[0] = {(unsigned int)(part_i), meta_part_out[0].meta(), partkernel[0]};
    mext_meta_part_out[1] = {(unsigned int)(part_i++), meta_part_out[1].meta(), partkernel[1]};

    for (int i = 0; i < 8; ++i) {
        mext_tab_part_out_col[i][0] = {(unsigned int)(part_i), tab_part_out_col[i][0], partkernel[0]};
        mext_tab_part_out_col[i][1] = {(unsigned int)(part_i++), tab_part_out_col[i][1], partkernel[1]};
    }
    cl_mem_ext_ptr_t mext_cfg_part = {(unsigned int)(part_i++), cfg_part, partkernel[0]};
    ...
    for (int k = 0; k < 2; k++) {
        for (int c = 0; c < 8; c++) {
            if (c < l_ncol) {
                std::cout<<"create Buffer buf_tab_part_in_col["<<c<<"]["<<k<<"]: size="<<tab_part_sec_size[c]<<std::endl;
                ...
                std::cout<<"create Buffer buf_tab_part_in_col["<<c<<"]["<<k<<"]: size="<<VEC_LEN<<std::endl;
                buf_tab_part_in_col[c][k] =
                    clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, VEC_LEN,
    ...
    for (int k = 0; k < 2; k++) {
        for (int c = 0; c < 8; c++) {
            if (c < l_ncol) {
                std::cout<<"create Buffer buf_tab_part_out_col["<<c<<"]["<<k<<"]: size="<<tab_part_out_col_size<<std::endl;
                buf_tab_part_out_col[c][k] =
                    clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                   tab_part_out_col_size, &mext_tab_part_out_col[c][k], &err);
            } else {
                std::cout<<"create Buffer buf_tab_part_out_col["<<c<<"]["<<k<<"]: size="<<VEC_LEN<<std::endl;
                buf_tab_part_out_col[c][k] =
                    clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, VEC_LEN,
    ...
    for (int k = 0; k < 2; k++) {
        clReleaseMemObject(buf_meta_part_in[k]);   
        clReleaseMemObject(buf_meta_part_out[k]);
    }

    for(int i=0;i<evt_part_h2d.size();i++)
        for(int j=0;j<evt_part_h2d[i].size();j++)
            clReleaseEvent(evt_part_h2d[i][j]);

    for(int i=0;i<evt_part_krn.size();i++)
        for(int j=0;j<evt_part_krn[i].size();j++)
            clReleaseEvent(evt_part_krn[i][j]);

    for(int i=0;i<evt_part_d2h.size();i++)
        for(int j=0;j<evt_part_d2h[i].size();j++)
            clReleaseEvent(evt_part_d2h[i][j]);

    for(int i=0;i<evt_part_memcpy_in.size();i++)
        for(int j=0;j<evt_part_memcpy_in[i].size();j++)
            clReleaseEvent(evt_part_memcpy_in[i][j]);

    for(int i=0;i<evt_part_memcpy_out.size();i++)
        for(int j=0;j<evt_part_memcpy_out[i].size();j++)
            clReleaseEvent(evt_part_memcpy_out[i][j]);

    for (int k = 0; k < 2; k++) {
        clReleaseKernel(partkernel[k]);
    }
    ...
    err = xf::database::gqe::load_binary(&prg, ctx, dev_id, (xclbinPath()+"/gqeAggr.xclbin").c_str());
    if (err != CL_SUCCESS) {
        fprintf(stderr, "ERROR: fail to program PL\n");
        exit(1);
    }
    ...
    for (int k = 0; k < 2; k++) {
        agg_i = 0;
        for (int c = 0; c < 8; c++) {
            mext_tab_aggr_in_col[c][k] = {(unsigned int)(agg_i++), tab_aggr_in_col[c][k], aggrkernel[k]};
        }
        mext_meta_aggr_in[k] = {(unsigned int)(agg_i++), meta_aggr_in[k].meta(), aggrkernel[k]};
        mext_meta_aggr_out[k] = {(unsigned int)(agg_i++), meta_aggr_out[k].meta(), aggrkernel[k]};
    }
    ...
    for (int k = 0; k < 2; k++) {
        for (int i = 0; i < 16; ++i) {
            clReleaseMemObject(buf_tab_aggr_out_col[i][k]);
        }
    }

    for(int i=0;i<evt_aggr_memcpy_in.size();i++)
        for(int j=0;j<evt_aggr_memcpy_in[i].size();j++)
            clReleaseEvent(evt_aggr_memcpy_in[i][j]);

    for(int i=0;i<evt_aggr_h2d.size();i++)
        for(int j=0;j<evt_aggr_h2d[i].size();j++)
            clReleaseEvent(evt_aggr_h2d[i][j]);

    for(int i=0;i<evt_aggr_krn.size();i++)
        for(int j=0;j<evt_aggr_krn[i].size();j++)
            clReleaseEvent(evt_aggr_krn[i][j]);

    for(int i=0;i<evt_aggr_d2h.size();i++)
        for(int j=0;j<evt_aggr_d2h[i].size();j++)
            clReleaseEvent(evt_aggr_d2h[i][j]);

    for(int i=0;i<evt_aggr_memcpy_out.size();i++)
        for(int j=0;j<evt_aggr_memcpy_out[i].size();j++)
            clReleaseEvent(evt_aggr_memcpy_out[i][j]);
