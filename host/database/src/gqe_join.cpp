...
Joiner::Joiner() {
    ...
    dev_id=Application::getDevice()();
    ctx=Application::getContext()();

    cq = clCreateCommandQueue(ctx, dev_id, CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &err);
    if (err != CL_SUCCESS) {
...
Joiner::~Joiner() {
    ...
    // if (err != CL_SUCCESS) {
    //     std::cout << "ERROR: fail to release program" << std::endl;
    //     exit(1);
    // }
...
ErrCode Joiner::join_sol0(Table& tab_a, Table& tab_b, Table& tab_c, JoinConfig& jcmd, std::vector<size_t> params) {
    ...
    for (int i = 0; i < 8; i++) {
        mext_table_o[i] = {(unsigned int)i, table_o[i], bkernel};
    }
    ...
    for (int j = 0; j < 8; j++) {
        mext_table_l[j] = {(unsigned int)j, table_l[j], jkernel};
        mext_table_out[j] = {(unsigned int)11 + j, table_out[j], jkernel};
    }

    mext_cfg5s = {19, table_cfg5s, bkernel};

    for (int c = 0; c < 16; c++) {
        memExt[0][c] = {(unsigned int)20 + c, nullptr, bkernel};
    }
    for (int c = 0; c < 16; c++) {
        memExt[1][c] = {(unsigned int)20 + c, nullptr, jkernel};
    }
...
ErrCode Joiner::join_sol1(Table& tab_a, Table& tab_b, Table& tab_c, JoinConfig& jcmd, std::vector<size_t> params) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 8; j++) {
            mext_table_l[i][j] = {(unsigned int)j, table_l[j][i], pkernel[i]};
            mext_table_out[i][j] = {(unsigned int)11 + j, table_out[j][i], pkernel[i]};
        }
    }
    ...
    for (int c = 0; c < 16; c++) {
        memExt[1][c] = {(unsigned int)20 + c, nullptr, pkernel[0]};
    }
...
// //////////////////////////////////////////////////
    ...
    //setup gqePart xclbin
    // std::cout<<"going to load gqePart xclbin file!"<<std::endl;
    // err = xf::database::gqe::load_binary(&prg, ctx, dev_id, (xclbinPath()+"/gqePart.xclbin").c_str());
    // if (err != CL_SUCCESS) {
    //     fprintf(stderr, "ERROR: fail to program PL\n");
    //     exit(1);
    // }
    prg=Application::getProgram<Lib::PART>()();
    // std::cout<<"loaded gqePart xclbin file!"<<std::endl;
    ...
    for (int i = 0; i < 8; ++i) {
        mext_table_o_partition_in_col[i][0] = {(unsigned int)(3 + i), table_o_partition_in_col[i][0], partkernel_O[0]};
        mext_table_o_partition_in_col[i][1] = {(unsigned int)(3 + i), table_o_partition_in_col[i][1], partkernel_O[1]};
    }
    ...
    for (int i = 0; i < 8; ++i) {
        mext_table_o_partition_out_col[i][0] = {(unsigned int)(13 + i), table_o_partition_out_col[i][0], partkernel_O[0]};
        mext_table_o_partition_out_col[i][1] = {(unsigned int)(13 + i), table_o_partition_out_col[i][1], partkernel_O[1]};
    }
...
//----------------------------partition O run-------------------------------
    ...
    for (int sec = 0; sec < table_o_sec_num; sec++) {
        ...
        // partEvents.push_back(evt_part_o_memcpy_out[sec][0]);
...
// //////////////////////////////////////////////////
    ...
    for (int i = 0; i < 8; ++i) {
        mext_table_l_partition_in_col[i][0] = {(unsigned int)(3 + i), table_l_partition_in_col[i][0], partkernel_L[0]};
        mext_table_l_partition_in_col[i][1] = {(unsigned int)(3 + i), table_l_partition_in_col[i][1], partkernel_L[1]};
    }
    ...
    for (int i = 0; i < 8; ++i) {
        mext_table_l_partition_out_col[i][0] = {(unsigned int)(13 + i), table_l_partition_out_col[i][0], partkernel_L[0]};
        mext_table_l_partition_out_col[i][1] = {(unsigned int)(13 + i), table_l_partition_out_col[i][1], partkernel_L[1]};
    }
...
//----------------------partition L run-----------------------------
    ...
    for (int sec = 0; sec < table_l_sec_num; sec++) {
        ...
        // partEvents.push_back(evt_part_l_memcpy_out[sec][0]);
    ...
    //     exit(1);
    // }
    // std::cout<<"released partition program"<<std::endl;
    ...
    //setup gqeJoin xclbin
    // std::cout<<"going to load gqeJoin xclbin file!"<<std::endl;
    // err = xf::database::gqe::load_binary(&prg, ctx, dev_id, (xclbinPath()+"/gqeJoin.xclbin").c_str());
    // if (err != CL_SUCCESS) {
    //     fprintf(stderr, "ERROR: fail to program PL\n");
    //     exit(1);
    // }
    prg=Application::getProgram<Lib::JOIN>()();
    // std::cout<<"loaded gqeJoin xclbin file!"<<std::endl;
    ...
    for (int i = 0; i < 8; i++) {
        mext_table_o_build_in_col[i] = {(unsigned int)i, table_o_build_in_col[i], bkernel};
    }

    for (int k = 0; k < 2; k++) {
        for (int i = 0; i < 8; i++) {
            mext_table_l_probe_in_col[i][k] = {(unsigned int)i, table_l_probe_in_col[i][k], pkernel[k]};
        }
    }
    mext_meta_build_in = {9, meta_build_in.meta(), bkernel};
    ...
    for (int k = 0; k < 2; k++) {
        for (int i = 0; i < 8; i++) {
            mext_table_l_probe_out_col[i][k] = {(unsigned int)(11 + i), table_l_probe_out_col[i][k], pkernel[k]};
        }
    }
    mext_cfg5s_hj = {19, q5s_cfg_join, bkernel};

    for (int i = 0; i < PU_NM * 2; ++i) {
        mext_hj_tmp[i] = {(unsigned int)(20 + i), nullptr, bkernel};
    }
    ...
    for (int i = 0; i < 8; i++) {
        if (i < o_valid_col_num) {
            ...
            // std::cout<<"buf_table_o_build_in_col["<<i<<"]: size="<<table_o_build_in_size_max<<std::endl;
        } else {
            buf_table_o_build_in_col[i] =
                clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 8,
                               &mext_table_o_build_in_col[i], &err);
            // std::cout<<"buf_table_o_build_in_col["<<i<<"]: size="<<8<<std::endl;
        }
    }
    ...
    for (int k = 0; k < 2; k++) {
        for (int i = 0; i < 8; i++) {
            if (i < l_valid_col_num) {
                ...
                // std::cout<<"buf_table_l_probe_in_col["<<i<<"]: size="<<table_l_probe_in_slice_size<<std::endl;    
            } else {
                buf_table_l_probe_in_col[i][k] =
                    clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 8,
                                   &mext_table_l_probe_in_col[i][k], &err);
                // std::cout<<"buf_table_l_probe_in_col["<<i<<"]: size="<<8<<std::endl;
            }
        }
    }
    ...
    for (int k = 0; k < 2; k++) {
        for (int i = 0; i < 8; i++) {
            if (i < out_valid_col_num) {
                ...
                // std::cout<<"buf_table_l_probe_out_col["<<i<<"]: size="<<table_l_probe_out_slice_size<<std::endl;
            } else {
                buf_table_l_probe_out_col[i][k] =
                    clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, 8,
                                   &mext_table_l_probe_out_col[i][k], &err);
                // std::cout<<"buf_table_l_probe_out_col["<<i<<"]: size="<<8<<std::endl;
            }
        }
    }
    ...
    // std::cout<<"buf_cfg5s_hj: size="<<(size_apu_512 * 9)<<std::endl;
    ...
    for (int i = 0; i < PU_NM; i++) {
        ...
        // std::cout<<"buf_hj_tmp["<<i<<"]: size="<<(size_t)(sizeof(int64_t) * HT_BUFF_DEPTH / 2)<<std::endl;
    }
    for (int i = PU_NM; i < PU_NM * 2; i++) {
        buf_hj_tmp[i] = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_HOST_NO_ACCESS | CL_MEM_EXT_PTR_XILINX,
                                       (size_t)(KEY_SZ * 2 * S_BUFF_DEPTH / 2), &mext_hj_tmp[i], &err);
        // std::cout<<"buf_hj_tmp["<<i<<"]: size="<<(size_t)(KEY_SZ * 2 * S_BUFF_DEPTH / 2)<<std::endl;
    }

    cl_mem buf_meta_build_in = clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                                              (size_apu_512 * 8), &mext_meta_build_in, &err);
    // std::cout<<"buf_meta_build_in: size="<<(size_apu_512 * 8)<<std::endl;
    ...
    // std::cout<<"buf_meta_probe_in[0]: size="<<(size_apu_512 * 8)<<std::endl;
    ...
    // std::cout<<"buf_meta_probe_in[1]: size="<<(size_apu_512 * 8)<<std::endl;
    cl_mem buf_meta_probe_out[2];
    buf_meta_probe_out[0] = clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                           (size_apu_512 * 8), &mext_meta_probe_out[0], &err);
    // std::cout<<"buf_meta_probe_out[0]: size="<<(size_apu_512 * 8)<<std::endl;
    buf_meta_probe_out[1] = clCreateBuffer(ctx, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                           (size_apu_512 * 8), &mext_meta_probe_out[1], &err);
    // std::cout<<"buf_meta_probe_out[1]: size="<<(size_apu_512 * 8)<<std::endl;
