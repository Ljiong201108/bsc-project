...
void ZstdDecompressWorkshop::process(){
    ...
    do{ //per frame
		...
		chunkWriterKernel->setArg(1, endBufferHost);
		chunkWriterKernel->setArg(2, (uint32_t)true);

		chunkWriterQueue->enqueueTask(*chunkWriterKernel);
		chunkWriterQueue->finish();

		chunkReaderQueue->enqueueMigrateMemObjects({*outputSizeBuffer, *statusBuffer, *outputBuffer}, CL_MIGRATE_MEM_OBJECT_HOST);
		chunkReaderQueue->finish();
...
//     KernelPointer chunkWriterKernel(Application::getProgram<Lib::ZSTD_DECOMPRESS>(), "xilMM2S:{xilMM2S_1}");
//     KernelPointer chunkReaderKernel(Application::getProgram<Lib::ZSTD_DECOMPRESS>(), "xilS2MM:{xilS2MM_1}");
...
// 	statusBufferHost[0]=0;
// 	std::thread chunkReader([this, &chunkReaderQueue, &chunkReaderKernel, &outputSizeBufferHost, &statusBufferHost, &outputBufferHost, &outputBuffer, &outputSizeBuffer, &statusBuffer]{
// 		do{
// 			// std::cout<<"start to read a chunk["<<std::dec<<outputBufferHost.size()<<" Bytes]"<<std::endl;
// 			chunkReaderQueue->enqueueMigrateMemObjects({*statusBuffer}, 0);

// 			std::cout<<"executing read task"<<std::endl;
// 			chunkReaderQueue->enqueueTask(*chunkReaderKernel);
...
// 			std::cout<<"executing write task"<<std::endl;
// 			chunkWriterQueue->enqueueTask(*chunkWriterKernel);
// 			chunkWriterQueue->finish();

// 			std::cout<<"write a chunk["<<chunkSize<<" Bytes], last = "<<last<<std::endl;
// 		}while(!last);
// 	});
...
void ZstdDecompressWorkshop::processALL(){
	CommandQueuePointer chunkWriterQueue(Application::getContext(), Application::getDevice(), CL_QUEUE_PROFILING_ENABLE);
    CommandQueuePointer chunkReaderQueue(Application::getContext(), Application::getDevice(), CL_QUEUE_PROFILING_ENABLE);
