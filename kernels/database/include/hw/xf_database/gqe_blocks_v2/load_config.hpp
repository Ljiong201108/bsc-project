...
#pragma HLS unroll
    ...
    if (join_on) {
        if (!build_probe_flag) {
            ...
            for(int t=0;t<CH_NM;t++) shuffle1_cfg_strm[t].write(shuffle_cfg1a);
        } else {
            ...
            for(int t=0;t<CH_NM;t++) shuffle1_cfg_strm[t].write(shuffle_cfg1b);
        }
