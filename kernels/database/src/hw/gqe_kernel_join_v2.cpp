...
void shuffle1_wrapper(hls::stream<bool>& join_on_strm,
                      hls::stream<ap_uint<8 * 8> > order_cfg[CH_NM],
                      hls::stream<ap_uint<8 * TPCH_INT_SZ> > istrms[CH_NM][8],
                      hls::stream<bool> e_istrm[CH_NM],
                      hls::stream<ap_uint<8 * TPCH_INT_SZ> > ostrms[CH_NM][8],
...
void load_scan_wrapper(bool build_probe_flag,
                       ...
                       hls::stream<ap_uint<8 * 8> > shuffle1_cfg_strm[CH_NM],
...
#pragma HLS stream variable = cid_A_strm depth = 32
    load_config<JN_NM, CH_NM>(build_probe_flag, tin_meta, buf_D, join_on_strm, join_dual_key_on_strm, agg_on_strm,
...
// #pragma HLS array_partition variable = shuffle0_cfg dim = 0
    ...
    hls::stream<ap_uint<8 * 8> > shuffle1_cfg[nch];
...
#pragma HLS stream variable = e_flt_strms depth = 32
    ...
    filter_ongoing<8, 8, nch>(join_on_strm[1].read(),
                            fcfg, //
                            ch_strms,
                            e_ch_strms, //
    ...
    hls::stream<ap_uint<8 * TPCH_INT_SZ> > hj_in[nch][8];
...
#pragma HLS resource variable = hj_in core = FIFO_LUTRAM
    hls::stream<bool> e_hj_in[nch];
#pragma HLS stream variable = e_hj_in depth = 32

    shuffle1_wrapper<nch>(join_on_strm[6], shuffle1_cfg, flt_strms, e_flt_strms, hj_in, e_hj_in);
    // printf("shuffle done\n");
...
#pragma HLS resource variable = flt_dm_strms_1 core = FIFO_LUTRAM
    ...
    hls::stream<bool> e_flt_dm_strms_0[nch];
#pragma HLS stream variable = e_flt_dm_strms_0 depth = 32
    hls::stream<bool> e_flt_dm_strms_1[nch];
