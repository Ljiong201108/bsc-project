...
#pragma HLS resource variable = config core = RAM_2P_URAM

    ap_uint<32> filter_cfg_a[filter_cfg_depth];
#pragma HLS resource variable = filter_cfg_a core = RAM_2P_URAM
...
#pragma HLS unroll
    ...
    xf::database::hashPartition<1, 64, 192, 32, HASHWH, HASHWL, 18, CH_NM, COL_OUT_NM>(
        mk_on_strm, k_depth_strm, bit_num_strm, key_strm, pld_strm, e_strm, o_bkpu_num_strm, o_nm_strm, out_strm, uram_bucket);
}
...
#pragma HLS INTERFACE m_axi offset = slave latency = 64 \
	num_write_outstanding = 16 num_read_outstanding = 1 \
	max_write_burst_length = 64 max_read_burst_length = 8 \
	bundle = gmem2_0 port = uram_bucket
...
#pragma HLS INTERFACE s_axilite port = uram_bucket bundle = control
...
#pragma HLS resource variable = hp_out_strms core = FIFO_SRL
// #pragma HLS resource variable = hp_out_strms core = FIFO_BRAM
    ...
    hash_partition_wrapper<COL_NUM, CH_NUM, COL_NUM>(mk_on_strm, k_depth_strm, bit_num_strm, flt_strms, e_flt_strms,
                                                     hp_bkpu_strm, hp_nm_strm, hp_out_strms, uram_bucket);
