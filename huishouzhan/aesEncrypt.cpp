...
void aesCbcEnc(const std::string &binaryFileName, uint64_t *plain, const uint64_t *key, const uint64_t *iv, uint64_t *cipher, uint32_t size){
    ...
    k_pointer.create(app.getProgram(), "aes"+KEY_LENGTH+"CbcEnc");
    BufferPointer &plain_pointer=bufferPool.get<0>().create(app.getContext(), CL_MEM_READ_WRITE, size*sizeof(uint64_t)*num64, NULL);
    BufferPointer &cipher_pointer=bufferPool.get<1>().create(app.getContext(), CL_MEM_READ_WRITE, size*sizeof(uint64_t)*num64, NULL);
