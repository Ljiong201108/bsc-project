...
XCLBINS=$(find . -name "*.xclbin")

package: $(LINK_OUTPUT) $(HOST_OUTPUT)
	v++ -p $(PAKG_FLG) $(LINK_OUTPUT) --package.out_dir package --package.rootfs $(ROOTFS)/rootfs.ext4 $(PAKG_SDFILE) --package.sd_file $(ROOTFS)/Image --package.sd_file xrt.ini --package.sd_file $(HOST_OUTPUT) $(patsubst %, --package.sd_file %, $(XCLBINS)) --package.sd_file run_on_zcu.sh --package.sd_file sample.txt
